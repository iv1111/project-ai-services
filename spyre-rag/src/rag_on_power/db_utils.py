...
class MilvusVectorStore:
    ...
    def insert_chunks(self, emb_model, emb_endpoint, max_tokens, chunks, deployment_type, batch_size=10):
        ...
        for i in tqdm(range(0, len(chunks), batch_size), desc='Ingesting Data into Vector DB'):
            ...
            self.metadata_map.extend([
                {"chunk_id": cid, "filename": fn, "type": t, "source": s, "page_content": pc}
                for cid, fn, t, s, pc in zip(chunk_ids, filenames, types, sources, page_contents)
            ])
    ...
    def search(self, query, emb_model, emb_endpoint, max_tokens, top_k=5, deployment_type='cpu', mode="hybrid"):
        self._ensure_embedder(emb_model, emb_endpoint, max_tokens, deployment_type)
        self.collection_name = self._generate_collection_name()
        ...
        if mode == "dense":
            ...
            sparse_results = [
                {**self.metadata_map[idx], "score": score}
            ...
            query_vec = self.vectorizer.transform([query])
            scores = (self.sparse_matrix @ query_vec.T).toarray().ravel()
            sparse_ranked = sorted(enumerate(scores), key=lambda x: x[1], reverse=True)[:top_k]

            sparse_results = [{**self.metadata_map[idx], "score": score} for idx, score in sparse_ranked]
