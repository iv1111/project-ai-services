...
def create_vector_db_from_directory(directory_path, emb_name, vlm_name, llm_name, reset_db, include_meta_info_in_main_text, deployment_type=DEPLOYMENT_TYPE, log_messages=''):
    # Initialize/reset the database before processing any files
    vector_store = MilvusVectorStore(db_prefix=DB_NAME_PREFIX, emb_name=emb_name, vlm_name=vlm_name, llm_name=llm_name)
    ...
    hierarchical_chunk_with_token_split(
        ...
        max_tokens=emb_model_dict[emb_name]['max_tokens'] - 100 if include_meta_info_in_main_text else emb_model_dict[emb_name]['max_tokens']
    )
    ...
    vector_store.insert_chunks(
        emb_model=emb_model_dict[emb_name]['emb_model'],
        emb_endpoint=emb_model_dict[emb_name]['emb_endpoint'],
        max_tokens=emb_model_dict[emb_name]['max_tokens'],
